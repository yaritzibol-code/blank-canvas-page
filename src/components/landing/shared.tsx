  ...
  const g: Record<IconName, ReactNode> = {
    ...
    compass: (
      <>
        ...
        <path d="M15 9l-2.2 6L9 16l2-6 4-1z" {...p} />
      </>
    ),
    target: (
      <>
    ...
    cards: (
      <>
        <rect x="4" y="7" width="13" height="13" rx="2.5" {...p} />
        <path d="M8 7V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h-2" {...p} />
      </>
    ),
    sim: (
      <>
    ...
    chat: (
      <path
        d="M4 6a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-4l-4 4v-4H6a2 2 0 0 1-2-2V6z"
        {...p}
      />
    ),
    audio: (
      <path d="M3 10v4a1 1 0 0 0 1 1h3l4 4V5L7 9H4a1 1 0 0 0-1 1zM16 8a5 5 0 0 1 0 8" {...p} />
    ),
    ...
    flame: (
      <path
        d="M12 3s4.5 4 4.5 8.5A4.5 4.5 0 1 1 7.5 11.5c0-2 1-3 2-4-1 4 2.5 4 2.5 7.5 0-3.5 4-3.5 4-7.5 0-3.5-4-4-4-4z"
        {...p}
      />
    ),
    ...
    grid: (
      <>
        <rect x="4" y="4" width="7" height="7" rx="1.4" {...p} />
        <rect x="13" y="4" width="7" height="7" rx="1.4" {...p} />
        <rect x="4" y="13" width="7" height="7" rx="1.4" {...p} />
        <rect x="13" y="13" width="7" height="7" rx="1.4" {...p} />
      </>
    ),
    cal: (
      <>
    ...
    alarm: (
      <>
        ...
        <path d="M12 9v4l2.5 2M5 3L2.5 6M19 3l2.5 3M9 21h6" {...p} />
      </>
    ),
    brain: (
      <path
        d="M9 4a3 3 0 0 0-3 3 3 3 0 0 0-1 5 3 3 0 0 0 2 4 3 3 0 0 0 5 1V4.5A2.5 2.5 0 0 0 9 4zM15 4a3 3 0 0 1 3 3 3 3 0 0 1 1 5 3 3 0 0 1-2 4 3 3 0 0 1-5 1"
        {...p}
      />
    ),
    heart: (
      <path
        d="M12 20s-7-4.3-9.3-8.2C1.2 9 2.3 5.5 5.5 5.1c2-.2 3.4 1 4.5 2.4 1.1-1.4 2.5-2.6 4.5-2.4 3.2.4 4.3 3.9 2.8 6.7C19 15.7 12 20 12 20z"
        {...p}
      />
    ),
...
export function Logo({ light = false, size = 30 }: { light?: boolean; size?: number }) {
  return (
    <Link
      to="/"
      aria-label="FlightPath — ir al inicio"
    ...
    >
      ...
      />
      <span
        className={`font-display text-[17px] sm:text-[19px] tracking-tight ${light ? "text-white" : "text-ink"}`}
      >
...
export function Btn({
  ...
  const inner = (
    <>
      {iconLeft && <Icon n={iconLeft} className="w-[18px] h-[18px]" />}
      {children}
      {icon && <Icon n={icon} className="w-[18px] h-[18px]" />}
    </>
  );
  ...
  if (to)
    return (
      <Link to={to} search={search as never} className={cls} onClick={onClick}>
        {inner}
      </Link>
    );
  if (href)
...
export function Eyebrow({ children, light = false }: { children: ReactNode; light?: boolean }) {
  return (
    <span
      className={`inline-flex items-center gap-2.5 text-[11px] uppercase tracking-[0.22em] font-bold ${light ? "text-white/55" : "text-haze-500"}`}
    >
...
export function Coord({ children, light = false }: { children: ReactNode; light?: boolean }) {
  return (
    <span
      className={`font-mono text-[10px] tracking-wide ${light ? "text-white/40" : "text-haze-400"}`}
    >
      {children}
    </span>
  );
...
export function AeroBackdrop({ theme = "hueso" }: { theme?: "hueso" | "cherry" | "azul" }) {
  return (
    <div
      className={`cloudscape cs-${theme} fixed inset-0 -z-10 overflow-hidden pointer-events-none`}
      aria-hidden="true"
    />
  );
}
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          {mounted && sesion ? (
            ...
            <>
              <div className="hidden md:block">
                <Btn kind="ghost" size="sm" to="/login">
                  Iniciar sesión
                </Btn>
              </div>
          ...
          >
            <span className="relative block w-[18px] h-[12px]" aria-hidden="true">
              <span
                className={`absolute left-0 w-full h-[2px] rounded bg-current transition-all duration-200 ${open ? "top-[5px] rotate-45" : "top-0"}`}
              />
              <span
                className={`absolute left-0 top-[5px] w-full h-[2px] rounded bg-current transition-all duration-200 ${open ? "opacity-0" : "opacity-100"}`}
              />
              <span
                className={`absolute left-0 w-full h-[2px] rounded bg-current transition-all duration-200 ${open ? "top-[5px] -rotate-45" : "top-[10px]"}`}
              />
            </span>
            <span className="hidden xs:inline text-[13px] font-semibold">
              {open ? "Cerrar" : "Menú"}
            </span>
          </button>
        </div>
        ...
        createPortal(
          ...
          >
            ...
            <nav className="mx-auto max-w-[1240px] px-4 sm:px-6 py-4 flex flex-col">
              <p className="text-[11px] font-bold uppercase tracking-wider text-ink/40 mb-1">
                Páginas
              </p>
              {NAV_LINKS.map((x) => (
                <a
        ...
        <div className="grid md:grid-cols-[1.6fr_1fr_1fr_1fr] gap-10 pb-12 border-b border-white/10">
          <div>
            ...
            <p className="mt-4 text-[11.5px] text-white/35 leading-relaxed max-w-xs">
              FlightPath es una plataforma independiente. No está afiliada a la AFAC ni al CIAAC, ni
              a ASPA de México, Aeroméxico, Volaris o ninguna otra aerolínea o institución.
            </p>
          ...
          {[
            ...
            <div key={i}>
              <div className="text-[11px] uppercase tracking-[0.18em] font-bold text-white/40">
                {c.h}
              </div>
              <ul className="mt-5 space-y-3">
                {c.l.map((x) => (
                  <li key={x.t}>
                    <a
                      href={x.href}
                      target={"ext" in x && x.ext ? "_blank" : undefined}
                      rel={"ext" in x && x.ext ? "noreferrer" : undefined}
                      className="text-[13.5px] text-white/70 hover:text-coral-400 transition-colors"
                    >
                      {x.t}
                    </a>
                  </li>
