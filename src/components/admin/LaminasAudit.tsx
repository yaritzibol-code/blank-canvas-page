  ...
  return (
    <div style={{ ...cardStyle, marginBottom: 20 }}>
      <div style={cardHeadStyle}>
        <Icon n="book" size={16} color="#C7A052" />
        <h3 style={{ fontSize: ".88rem", fontWeight: 800, color: "#FFFFFF" }}>Láminas de las preguntas</h3>
      </div>

      {!rows && !error && <p style={{ fontSize: ".8rem", color: "#93A4BF" }}>Revisando módulos…</p>}
      {error && <p style={{ fontSize: ".8rem", color: "#c0392b" }}>No se pudo revisar: {error}</p>}

      {rows && conLaminas.length === 0 && (
        <p style={{ fontSize: ".8rem", color: "#93A4BF" }}>Ningún módulo tiene preguntas con figura.</p>
      )}
      ...
      {conLaminas.map((r) => {
        ...
        return (
          <div
            key={r.fuente}
            style={{ display: "flex", alignItems: "center", gap: 10, fontSize: ".78rem", borderBottom: "1px solid rgba(255,255,255,.06)", paddingBottom: 7, marginBottom: 7 }}
          >
            <span style={{ flex: 1, color: "#93A4BF" }}>{NOMBRE[r.fuente] ?? r.fuente}</span>
            <span style={{ color: "#93A4BF" }}>{Number(r.con_lamina)} preguntas con figura</span>
            <strong style={{ color: faltan === 0 ? "#FFFFFF" : "#c0392b" }}>
      ...
      {rows && conLaminas.some((r) => Number(r.laminas_distintas) > Number(r.laminas_existentes)) && (
        <p style={{ fontSize: ".74rem", color: "#93A4BF", marginTop: 6 }}>
